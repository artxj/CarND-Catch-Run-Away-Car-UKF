...
*/
void UKF::PredictPosition(double distance) {
  // some dt got empirically
  double delta_t = sqrt(distance) / 2.0;

  if (distance < 2.0) {
    // distance is small enough, target can be heading right here
    if (distance > 1e-3) return;

    double new_distance = sqrt(pow(x_[0] - x_pred_[0], 2) + pow(x_[1] - x_pred_[1], 2));
    if (actual_distance_ - new_distance > -0.1) {
      // distance is decreasing, so let's wait for target in this point
      actual_distance_ = new_distance;
      return;
    } else {
      // distance is increasing, seems like we missed the target
      // let's hunt for it again!
      delta_t = 1.0;
    }
  } else {
    // big enough
    actual_distance_ = 10000;
  }
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; ++i) {
    ...
    Xsig_pred.col(i) = x_out;
  }

  // predicted state mean
  x_pred_.setZero();
  for (int i = 0; i < 2 * n_aug_ + 1; ++i) {
    x_pred_ += weights_(i) * Xsig_pred.col(i);
  }
  x_pred_(3) = tools.NormalizeAngle(x_pred_(3));
  // std::cout << "Predicted position: " << x_pred_ << std::endl;
}
