...
int main()
{
  ...
  double target_x = 0.0;
  double target_y = 0.0;
  double distance_difference = 20;

  h.onMessage([&ukf,&target_x,&target_y,&distance_difference](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        //std::cout << distance_difference << std::endl;
        ukf.PredictPosition(distance_difference);
	  target_x = ukf.x_pred_[0];
	  target_y = ukf.x_pred_[1];
